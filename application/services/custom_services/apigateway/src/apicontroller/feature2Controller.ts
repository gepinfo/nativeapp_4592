...
export class feature2Controller implements Controller {
    ...
    private initializeRoutes() {
        this.router.post('/feature2', this.GpCreate);
this.router.get('/feature2/userid/created_by', this.GpGetNounCreatedBy);
        //#@gepdelimeterone@#
        //#@ssofacebookapiroute@#
        //#@ssogithubapiroute@#
        //#@gepbankingapiroute@#
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.FEATURE2URL + `${req.url}` , req.body)
...
public GpGetNounCreatedBy(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.FEATURE2URL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
